...
const Navbar = ({ onOpenMegaMenu, isLightPage }: { onOpenMegaMenu: () => void, isLightPage?: boolean }) => {
  ...
  return (
    <nav ref={navRef} className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${useDarkTheme ? 'bg-white shadow-xl py-2' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => {
          ...
          <img src={IMAGES.logo} alt="DPR Logo" className={`h-20 w-auto ${!useDarkTheme ? 'brightness-0 invert' : ''}`} referrerPolicy="no-referrer" />
        </div>
        ...
        <div className="hidden lg:flex items-center gap-8">
          {NAV_LINKS.map((link) => (
            ...
            >
              <a 
                ...
                className={`flex items-center gap-1 font-semibold text-sm uppercase tracking-wider transition-colors cursor-pointer ${useDarkTheme ? 'text-slate-900 hover:text-blue-700' : 'text-white hover:text-blue-300'}`}
              >
                {link.title}
          ...
          <button 
            ...
            className={`p-2 rounded-full transition-colors ${useDarkTheme ? 'text-slate-900 hover:bg-slate-100' : 'text-white hover:bg-white/10'}`}
          >
            <Menu size={24} />
          </button>
        </div>
        ...
        <div className="lg:hidden">
           <button onClick={onOpenMegaMenu} className={useDarkTheme ? 'text-slate-900' : 'text-white'}>
             <Menu size={28} />
           </button>
        </div>
      </div>
        ...
        >
          ...
          <div className="flex flex-wrap gap-4">
            <button onClick={() => {
                ...
                window.dispatchEvent(new PopStateEvent('popstate'));
              }} className="px-8 py-4 bg-blue-700 hover:bg-blue-800 text-white font-bold rounded-sm transition-all flex items-center gap-2 group">
          ...
          {projects.map((project, i) => (
            ...
            >
              ...
              <div className="p-8">
                ...
                <p className="text-slate-600 leading-relaxed text-justify">{project.desc}</p>
              </div>
            </motion.div>
          ))}
        </div>
          ...
          {data.projects.map((project, i) => (
            ...
            >
              ...
              <div className="p-8">
                ...
                <p className="text-slate-600 leading-relaxed text-justify">{project.details}</p>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center gap-2 text-sm text-slate-500">
          ...
          {window.location.pathname.startsWith('/projects') ? (
            <button 
              ...
              className="hover:text-blue-600 transition-colors text-slate-900 font-medium"
            >
              Projects
            </button>
          ) : (
      ...
      <section className="py-16 md:py-24">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-3 gap-16">
          <div className="lg:col-span-2 space-y-12">
            ...
            {(article as any).purpose && (
              <div className="pt-6 border-t border-slate-100">
                <h2 className="text-2xl font-bold mb-6 text-slate-900">Project Purpose</h2>
            ...
            <div className="pt-12 border-t border-slate-200">
              <button 
                onClick={() => {
                  ...
                  window.history.pushState({}, '', isProjects ? '/projects' : '/');
                  window.dispatchEvent(new PopStateEvent('popstate'));
                }}
                className="flex items-center gap-2 text-blue-700 font-bold hover:gap-4 transition-all"
              >
                <ArrowLeft size={20} />
                {window.location.pathname.startsWith('/projects') ? 'Back to Projects' : 'Back to News & Insights'}
              </button>
            </div>
          </div>
          ...
          <div className="space-y-8">
            <div className="bg-slate-50 p-8 rounded-2xl border border-slate-200">
              ...
              <div className="space-y-6">
                ...
                {(article as any).implementingAgency && (
                  <div>
                    <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Implementing Agency</div>
                ...
                {(article as any).companies && (article as any).companies.length > 0 && (
                  <div>
                    ...
                    <ul className="text-slate-900 font-medium text-sm space-y-1.5 list-disc list-inside">
                      {(article as any).companies.map((company: string, idx: number) => (
                        <li key={idx} className="leading-snug">{company}</li>
                      ))}
                    </ul>
                  </div>
                )}
...
export default function App() {
  ...
  return (
    <div className="font-sans text-slate-900 bg-white selection:bg-blue-100 selection:text-blue-900">
      <Navbar onOpenMegaMenu={() => setIsMegaMenuOpen(true)} isLightPage={isLightPage} />
      ...
      {currentPage === 'projects' && <ProjectGallery />}
